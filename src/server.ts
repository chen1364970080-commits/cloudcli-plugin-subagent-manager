...
function parseAgentLine(line: string): Record<string, unknown> | null {
  try {
...
function detectStatus(entries: Record<string, unknown>[]): 'running' | 'complete' | 'error' {
  ...
  if (Array.isArray(content)) {
    ...
    if (stopReason === null && last['type'] === 'assistant') return 'running';
  }
  return 'complete';
}

function getStopReason(obj: Record<string, unknown>): string | null {
  const msg = obj['message'] as Record<string, unknown> | undefined;
  if (!msg) return null;
  return (msg['stop_reason'] as string | null) ?? null;
}

function getModel(entry: Record<string, unknown>): string {
  const msg = entry['message'] as Record<string, unknown> | undefined;
  if (!msg) return 'unknown';
  return (msg['model'] as string) ?? 'unknown';
}

function getFirstUserMessage(entries: Record<string, unknown>[]): string {
      ...
      if (Array.isArray(content)) {
        for (const block of content) {
          if (block && typeof block === 'object' && (block as Record<string, unknown>)['type'] === 'text') {
            ...
            if (text.trim()) return truncate(text, 120);
          }
        }
      }
    }
...
function getAgentInfo(filePath: string): AgentEntry | null {
  let content: string;
  let fileSize = 0;
  try {
  ...
  const cwd = (first['cwd'] as string) ?? '';
  const sessionId = (first['sessionId'] as string) ?? '';
  const slug = (first['slug'] as string) ?? '';

  const startTime = new Date((first['timestamp'] as string) ?? Date.now()).getTime();
  const lastTime = new Date((last['timestamp'] as string) ?? Date.now()).getTime();
...
function getAgentsForProject(projectPath: string): AgentEntry[] {
...
function getAgentList(projectPath?: string): { agents: AgentEntry[]; projectPath: string | null } {
  if (!projectPath) {
